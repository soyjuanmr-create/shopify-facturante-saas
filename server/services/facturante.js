...
class FacturanteService {
  ...
  formatearItems(items) {
    return items.map(function (item, idx) {
      ...
      var total = Math.round(pu * (1 - bon / 100) * cant * (1 + alic / 100) * 1000) / 1000;
  ...
  async crearComprobante(facturaData, webhookUrl) {
    ...
    var nroDoc = (cliente.nro_documento || '0').toString().replace(/\D/g, '') || '0';
